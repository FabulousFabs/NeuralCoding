...
# load current pass
with open(os.path.join(mod, 'phase_clean_i.npy'), 'rb') as f:
    last_pass = np.load(f)

# load model
with snn.Network(build_from = os.path.join(mod, 'phase_clean_p{:d}.npy'.format(last_pass))) as net:
    ...
    fib_excite = net.fibre(pre = pop_in, post = pop_ro, type = snn.synapses.Full(generator = snn.generators.Xavier, n = 50, directionality = 1), plasticity = snn.plasticity.STDP(lr = 1e-6), filter = snn.synapses.filters.Phase(phases = bits))
...
for EPOCH in np.arange(0, EPOCHS, 1):
    for i, stimulus in zip(np.arange(0, len(stimuli), 1), stimuli):
        ...
        s = stimulus.split('.')[0]
        with open(os.path.join(bod, 'phase_clean_e{:d}_{:s}.npy'.format(last_pass, s)), 'wb') as f:
            np.save(f, sim.monitors[mon].state)

    # save model trained as per current epoch
    last_pass = last_pass + 1
    net.save(to = os.path.join(mod, 'phase_clean_p{:d}.npy'.format(last_pass)))

    # save pass
    with open(os.path.join(mod, 'phase_clean_i.npy'), 'wb') as f:
        np.save(f, last_pass)
