...
# enter loop
print('Starting rate encoding of stimuli.')
...
for i in np.arange(0, len(targets), 1):
    ...
    Ek = Ek / np.max(Ek) * Hz_max

    # compute the rate response
    M[:,0:Ek.shape[1]] = snn.utils.neuralcoding.rate(inputs = Ek, homogenous = False, L = Ek.shape[1], lam = rate_lambda)
